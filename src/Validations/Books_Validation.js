...
const book_validatedata = async (body, option,res) => {
    try {
        let validation;
            ...
            case 'book_filter':
                switch(body.choice){
                    case 'all':validation_result = {status:true};
                        break;
                    case 'title':
                        validation = new Validator(body, book_filter.book_title, errmsg.book_filter.title)
                        validation_result = validation.fails() ? { status: false, message: 'Validation Unsuccessfull', validation: validation.errors.errors } : { status: true };
                        break;
                    case 'id':
...
export default book_validatedata
